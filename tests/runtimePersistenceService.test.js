...
test('runtime persistence service uses the typed adapter against a native SQLite file', { skip: !createRuntimePersistenceService }, async () => {
  ...
  try {
    ...
    const operational = await service.execute('branch.get_operational_data', { branch_id: 'b1' });
    const catalog = await service.execute('catalog.get', { branch_id: 'b1' });
    const inventory = await service.execute('inventory.get_availability', { branch_id: 'b1' });
